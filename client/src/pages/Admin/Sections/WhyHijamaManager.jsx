...
const WhyHijamaManager = () => {
  ...
  const handleSave = async (e = null, isAuto = false) => {
    ...
    try {
      if (selectedId) {
        ...
        // Instantly notify standard layouts to live preview refreshed elements!
  ...
  // Intelligent debounce auto-save controller 💓
  useEffect(() => {
    ...
    benefit_5_icon, benefit_5_title, benefit_5_desc,
    benefit_6_icon, benefit_6_title, benefit_6_desc,
    journey_title,
  ...
  return (
    <div className="admin-section">
      ...
      {showForm ? (
        ...
        <>
          <GlobalHeadingEditor slug="why_hijama" />
            ...
            {loading ? (
              ...
              <div className="admin-info-card clickable-setup-card" key={item.id} onClick={() => openEdit(item)} style={{ cursor: 'pointer', transition: 'transform 0.2s ease' }}>
                <div className="admin-info-row">
                  <span className="admin-info-label">Journey Title</span>
                  <div className="admin-info-value" style={{ fontWeight: '600', color: '#0f172a' }}>
                    {item.journey_title || '-'}
                  </div>
                </div>
                ...
                <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9', background: '#fafafa' }}>
                  <div className="admin-info-label" style={{ marginBottom: '12px' }}>Core Benefits (6 Columns)</div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(140px, 1fr))', gap: '10px' }}>
                    {[1, 2, 3, 4, 5, 6].map(num => {
                      const iconName = item[`benefit_${num}_icon`];
                      const title = item[`benefit_${num}_title`];
                      const Icon = (iconName && iconName !== 'Icon' && LucideIcons[iconName]) || LucideIcons.CheckCircle2;
                      return (
                        <div key={num} style={{ display: 'flex', flexDirection: 'column', background: '#fff', padding: '10px', borderRadius: '10px', border: '1px solid #e2e8f0' }}>
                          <div style={{ color: '#0891b2', marginBottom: '6px' }}><Icon size={16} /></div>
                          <div style={{ fontSize: '11px', fontWeight: '700', lineHeight: '1.3', color: '#1e293b' }}>{title || `Benefit ${num}`}</div>
                        </div>
                      );
                    })}
                  </div>
                ...
                <div style={{ padding: '16px 24px', borderBottom: '1px solid #f1f5f9' }}>
                  <div className="admin-info-label" style={{ marginBottom: '12px' }}>Journey Steps Timeline</div>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                    {[1, 2, 3, 4, 5].map(num => {
                      const title = item[`step_${num}_title`];
                      const desc = item[`step_${num}_desc`];
                      return (
                        <div key={num} style={{ display: 'flex', gap: '12px', alignItems: 'center', padding: '8px 12px', background: '#f8fafc', borderRadius: '8px' }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#e0f2fe', color: '#0369a1', width: '24px', height: '24px', borderRadius: '50%', fontSize: '11px', fontWeight: '800', flexShrink: '0' }}>{num}</div>
                          <div style={{ overflow: 'hidden' }}>
                            <div style={{ fontSize: '12px', fontWeight: '600', color: '#334155', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{title || `Step ${num}`}</div>
                            {desc && <div style={{ fontSize: '11px', color: '#64748b', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{desc}</div>}
                          </div>
                        </div>
                      );
                    })}
              ...
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px', color: '#94a3b8', background: '#ffffff', borderRadius: '16px', border: '1px dashed #e2e8f0' }}>
                No Why Hijama configurations configured yet.
              </div>
            )}
          </div>
        </>
