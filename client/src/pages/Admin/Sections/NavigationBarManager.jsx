...
const NavigationBarManager = () => {
  ...
  // Form State
  const [site_icon_, setSite_icon_] = useState('');
  const [isDropdownOpen_site_icon_, setIsDropdownOpen_site_icon_] = useState(false);
  const [site_heading, setSite_heading] = useState('');
  const [book_now_button_, setBook_now_button_] = useState('');
  const [status, setStatus] = useState('Active');

  const loadData = async () => {
    setLoading(true);
    try {
      const result = await crudService.getAll('navigation_bar');
  ...
  const handleSave = async (e = null, isAuto = false) => {
    ...
    formData.append('site_icon_', site_icon_);
    formData.append('site_heading', site_heading);
    formData.append('book_now_button_', JSON.stringify(book_now_button_));
    formData.append('status', status);

    try {
      if (selectedId) {
        await crudService.update('navigation_bar', selectedId, formData);
        if (!isAuto) showToast('Updated successfully');
        // Refresh visual simulator hot loads
        window.dispatchEvent(new CustomEvent('api-data-updated'));
      } else {
        await crudService.create('navigation_bar', formData);
  ...
  // Intelligence debounce engine for Navigation settings 💓
  useEffect(() => {
  ...
  }, [site_icon_, site_heading, book_now_button_, status]);

  const handleDelete = async (id) => {
    try {
      await crudService.delete('navigation_bar', id);
  ...
  const openEdit = (item) => {
    ...
    if (item.site_icon_ !== undefined && item.site_icon_ !== null) {
      setSite_icon_(item.site_icon_);
    } else {
      setSite_icon_('');
    }
    if (item.site_heading !== undefined && item.site_heading !== null) {
      setSite_heading(item.site_heading);
    } else {
      setSite_heading('');
    }
    if (item.book_now_button_ !== undefined && item.book_now_button_ !== null) {
      if (typeof item.book_now_button_ === 'object') { setBook_now_button_(item.book_now_button_); } 
      else { try { setBook_now_button_(JSON.parse(item.book_now_button_)); } catch(e) { setBook_now_button_(''); } }
    } else {
      setBook_now_button_('');
    }
  ...
  return (
    <div className="admin-section">
      <div className="admin-card-header">
        <div>
          <h2 className="admin-card-title">Navigation Bar </h2>
          <p className="admin-card-subtitle">Manage your Navigation Bar  section data here.</p>
        </div>
        ...
        <div className="admin-card">
          <form onSubmit={handleSave}>
            ...
            <div className="admin-form-group">
              <label className="admin-label">Site icon </label>
              <div className="icon-selector-premium">
                <div className="icon-current" onClick={(e) => {
                  e.stopPropagation();
                  setIsDropdownOpen_site_icon_(!isDropdownOpen_site_icon_);
                }}>
                  {(() => { 
                    const iconName = site_icon_;
                    const brandIcons = {
                  ...
                  <span>{ site_icon_ || 'Select Icon' }</span>
                  <LucideIcons.ChevronDown size={14} style={{ marginLeft: 'auto' }} />
                </div>
                {isDropdownOpen_site_icon_ && (
                  <div id="icon_dropdown_site_icon_" className="icon-dropdown-grid active">
                    ...
                    <div className="icon-grid-scroll">
                      {[
                        ...
                        return (
                          <div 
                            key={iconName} 
                            className={'icon-grid-item ' + (site_icon_ === iconName ? 'active' : '')}
                            data-name={iconName}
                            onClick={() => {
                              setSite_icon_(iconName);
                              setIsDropdownOpen_site_icon_(false);
                            }}
                          >
            ...
            <div className="admin-form-group">
              <label className="admin-label">Site Heading</label>
              <div className="input-wrapper-premium">
                
                <input type="text"  className="admin-input" value={ site_heading } onChange={(e) => setSite_heading(e.target.value)} placeholder="Enter Site Heading..." />
                
              </div>
            </div>
            <div className="admin-form-group" style={{ gridColumn: '1 / -1' }}>
              <label className="admin-label">Book Now Button </label>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: '12px' }}>
                <input type="text" className="admin-input" value={ book_now_button_?.label || '' } onChange={(e) => setBook_now_button_({ ...book_now_button_, label: e.target.value })} placeholder="Button Label (e.g. Learn More)" />
                <input type="text" className="admin-input" value={ book_now_button_?.url || '' } onChange={(e) => setBook_now_button_({ ...book_now_button_, url: e.target.value })} placeholder="Link URL (e.g. https://...)" />
              </div>
            </div>
            </div>
            ...
            {loading ? (
              ...
              <div className="admin-info-card clickable-setup-card" key={item.id} onClick={() => openEdit(item)} style={{ cursor: 'pointer', transition: 'transform 0.2s ease' }}>
                <div className="admin-info-row">
                  <span className="admin-info-label">Site Icon</span>
                  <div className="admin-info-value">
                    <div className="admin-card-icon-box">
                      {(() => { 
                        const iconName = item.site_icon_;
                        const brandIcons = {
                ...
                <div className="admin-info-row">
                  <span className="admin-info-label">Site Heading</span>
                  <div className="admin-info-value font-bold" style={{ fontWeight: '600' }}>{item.site_heading || '-'}</div>
                </div>
                <div className="admin-info-row">
                  <span className="admin-info-label">Book Button</span>
                  <div className="admin-info-value">
                    {(() => { 
                      try { 
                        const link = typeof item.book_now_button_ === 'string' ? JSON.parse(item.book_now_button_) : item.book_now_button_; 
                        return link ? <a href={link.url} target="_blank" rel="noopener noreferrer" style={{ color: '#4f46e5', fontWeight: '600', textDecoration: 'none' }}>{link.label || 'Link'}</a> : '-'; 
                      } catch(e) { return '-'; } 
                    })()}
                  </div>
                </div>
              ...
              <div style={{ gridColumn: '1/-1', textAlign: 'center', padding: '60px', color: '#94a3b8', background: '#ffffff', borderRadius: '16px', border: '1px dashed #e2e8f0' }}>
                No navigation config found. Click "Add New" if visible to configure.
              </div>
            )}
          </div>
      )}
