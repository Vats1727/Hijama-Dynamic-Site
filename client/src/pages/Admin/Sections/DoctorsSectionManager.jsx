...
const DoctorsSectionManager = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const result = await crudService.getAll('doctors_section');
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (selectedId) {
        await crudService.update('doctors_section', selectedId, formData);
        showToast('Updated successfully');
      } else {
        await crudService.create('doctors_section', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await crudService.delete('doctors_section', id);
  ...
  return (
    <div className="admin-section">
      <div className="admin-card-header">
        <div>
          <h2 className="admin-card-title">Doctors Section</h2>
          <p className="admin-card-subtitle">Manage your Doctors Section section data here.</p>
        </div>
        {!showForm && (
...
export default DoctorsSectionManager;
