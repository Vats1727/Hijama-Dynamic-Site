...
const Services_sectionManager = () => {
  ...
  const loadData = async () => {
    setLoading(true);
    try {
      const result = await crudService.getAll('services_section');
  ...
  const handleSave = async (e) => {
    ...
    formData.append('desc_', desc_);
    formData.append('status', status);

    try {
      if (selectedId) {
        await crudService.update('services_section', selectedId, formData);
        showToast('Updated successfully');
      } else {
        await crudService.create('services_section', formData);
  ...
  const handleDelete = async (id) => {
    try {
      await crudService.delete('services_section', id);
  ...
  const openEdit = (item) => {
    ...
    if (item.desc_ !== undefined && item.desc_ !== null) {
      setDesc_(item.desc_);
    } else {
      setDesc_('');
    }
  ...
  return (
    <div className="admin-section">
      <div className="admin-card-header">
        <div>
          <h2 className="admin-card-title">services_section</h2>
          <p className="admin-card-subtitle">Manage your services_section section data here.</p>
        </div>
        {!showForm && (
      ...
      {showForm ? (
        <div className="admin-card">
          <form onSubmit={handleSave}>
            ...
            <div className="admin-form-group">
              <label className="admin-label">desc_</label>
              <textarea className="admin-input" rows="4" value={ desc_ } onChange={(e) => setDesc_(e.target.value)} placeholder="Enter desc_..."></textarea>
            </div>
            </div>
...
export default Services_sectionManager;
